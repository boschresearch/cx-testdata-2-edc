...
def create_policy(asset_id: str):
    ...
    r = requests.post(f"{PROVIDER_EDC_BASE_URL}/policies", json=data, headers=prepare_data_management_auth())
    return policy_id
...
def edc_is_clean():
    ...
    nr_of_policies = get_number_of_elements(f"{PROVIDER_EDC_BASE_URL}/policies")
...
def test_create_and_delete(edc_is_clean: bool):
    ...
    nr_of_elements = get_number_of_elements(f"{PROVIDER_EDC_BASE_URL}/policies")
    assert nr_of_elements == NR_OF_ASSETS
    ...
    # delete policy
    r = requests.delete(f"{PROVIDER_EDC_BASE_URL}/policies/{first['policy_id']}", headers=prepare_data_management_auth())
    assert str(r.status_code).startswith('20'), f"Response code NOT 20x. status_code: {r.status_code}"
    nr_of_elements = get_number_of_elements(f"{PROVIDER_EDC_BASE_URL}/policies")
    assert nr_of_elements == NR_OF_ASSETS - 1
